...
const LegalComplianceDashboard: React.FC = () => {
  ...
  const loadComplianceData = async () => {
    try {
      ...
      const statusResponse = await fetch('http://localhost:3001/api/legal/compliance/status');
      const statusData = await statusResponse.json();
      setComplianceStatus(statusData);

      // Load recent journal entries
      const journalResponse = await fetch('http://localhost:3001/api/legal/journal/entries?limit=10');
      const journalData = await journalResponse.json();
      setJournalEntries(journalData.entries);

      // Load closure bulletins
      const closuresResponse = await fetch('http://localhost:3001/api/legal/closure/bulletins?type=DAILY');
      const closuresData = await closuresResponse.json();
      setClosureBulletins(closuresData.bulletins);
