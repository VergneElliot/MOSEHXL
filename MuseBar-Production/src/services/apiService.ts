...
export class ApiService {
  ...
  // Legal and Scheduler methods
  async getLegalSchedulerStatus(): Promise<any> {
    return this.request('/legal/scheduler/status');
  }

  async updateClosureSettings(settings: any): Promise<any> {
    return this.request('/legal/settings/closure', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  async triggerManualClosure(): Promise<any> {
    return this.request('/legal/scheduler/trigger', {
      method: 'POST',
    });
  }
}
