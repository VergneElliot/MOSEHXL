...
function parseKitchenPrinterIds(value: unknown): number[] | undefined {
  if (value === undefined) return undefined;
  if (!Array.isArray(value)) throw new ValidationError('kitchen_printer_ids must be an array');
  const ids = value.map((entry) => {
    const id = Number(entry);
    if (!Number.isInteger(id) || id < 1) throw new ValidationError('kitchen_printer_ids contains an invalid id');
    return id;
  });
  return [...new Set(ids)];
}
...
router.get('/', readCatalog, asyncHandler(async (req, res) => {
  ...
  try {
    ...
    res.json(await enrichProductsWithOperationalCatalog(products, establishmentId));
  } catch {
...
router.get('/:id', readCatalog, validateParams([paramValidations.id]), asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const [enriched] = await enrichProductsWithOperationalCatalog([product], establishmentId);
    res.json(enriched);
  } catch {
...
router.post('/', menuWrite, asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (parsedKitchenPrinterIds) {
      await KitchenPrinterModel.setProductAssignments(product.id, parsedKitchenPrinterIds, establishmentId);
    }
    const [enriched] = await enrichProductsWithOperationalCatalog([product], establishmentId);
    await logAuditOrThrow({
      ...
      action_details: { name, price, tax_rate, category_id, option_group_ids: parsedOptionGroupIds, kitchen_printer_ids: parsedKitchenPrinterIds, establishmentId },
      ip_address: req.ip,
...
router.put('/:id', menuWrite, validateParams([paramValidations.id]), asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (Object.keys(updateData).length === 0 && parsedOptionGroupIds === undefined && parsedKitchenPrinterIds === undefined) {
    ...
    if (parsedKitchenPrinterIds !== undefined) {
      await KitchenPrinterModel.setProductAssignments(id, parsedKitchenPrinterIds, establishmentId);
    }
    const [enriched] = await enrichProductsWithOperationalCatalog([product], establishmentId);
    await logAuditOrThrow({
      user_id: String(req.user!.id),
      ...
      action_details: { ...updateData, option_group_ids: parsedOptionGroupIds, kitchen_printer_ids: parsedKitchenPrinterIds },
      ip_address: req.ip,
      user_agent: req.headers['user-agent'],
    }, 'UPDATE_PRODUCT');
    res.json(enriched);
