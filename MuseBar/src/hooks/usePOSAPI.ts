...
export const usePOSAPI = (
  ...
  const processRetour = useCallback(async (retourData: RetourData) => {
    try {
      const response = await apiService.post('/orders', {
      ...
      const errorMessage = error.response?.data?.error || error.message || 'Erreur lors du traitement du retour';
      onError(errorMessage);
      throw error;
    }
  }, [apiService, onSuccess, onError, onDataUpdate]);

  const processChange = useCallback(async (changeData: ChangeData) => {
    try {
      const notes = changeData.direction === 'card-to-cash' 
      ...
      const errorMessage = error.response?.data?.error || error.message || 'Erreur lors du changement de caisse';
      onError(errorMessage);
      throw error;
    }
  }, [apiService, onSuccess, onError, onDataUpdate]);
