...
const HappyHourControl: React.FC<HappyHourControlProps> = ({
  isActive,
  timeUntil,
  onStatusUpdate,
}) => {
  ...
  const [editForm, setEditForm] = useState<{ type: 'percentage' | 'fixed'; value: string }>({
    type: 'percentage',
    value: '0',
  });
  ...
  useEffect(() => {
    ...
    setSettings({
      ...
      discountValue:
        currentSettings.discountType === 'fixed'
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={6}>
                  <TextField
                    ...
                    onChange={e =>
                      setSettings(prev => ({ ...prev, discountValue: Number(e.target.value) }))
                    }
                    fullWidth
                    inputProps={
                      settings.discountType === 'percentage'
          ...
          <Card>
            <CardContent>
              ...
              {eligibleProducts.length === 0 ? (
                <Alert severity="info">
                  Aucun produit éligible à l'Happy Hour. Vous pouvez configurer l'éligibilité dans
                  la gestion du menu.
                </Alert>
              ) : (
                <Grid container spacing={2}>
                  {eligibleProducts.map(product => {
                    const value =
                      typeof product.happyHourDiscountValue === 'number'
                        ? product.happyHourDiscountValue
                        : 0;
                    const label =
                        ...
                        <Paper sx={{ p: 2 }}>
                          ...
                          {!isEditing ? (
                            ...
                            <Box sx={{ mt: 1 }}>
                              ...
                              <TextField
                                label={
                                  editForm.type === 'percentage' ? 'Réduction (%)' : 'Réduction (€)'
                                }
                                type="number"
                                fullWidth
                              ...
                              <Box sx={{ display: 'flex', gap: 1 }}>
                                <Button
                                  ...
                                  onClick={async () => {
                                    ...
                                    setEligibleProducts(
                                      products.filter(
                                        (p: Product) => p.isHappyHourEligible && p.isActive
                                      )
                                    );
                                    onStatusUpdate();
                                  }}
                                ...
                                <Button size="small" onClick={() => setEditingProductId(null)}>
                                  Annuler
                                </Button>
                              </Box>
                            </Box>
