...
const LegalReceipt: React.FC<LegalReceiptProps> = ({
  order,
  businessInfo,
  ...
  const sousTotalHT =
    !isNaN(parseFloat(String(order.total_amount))) && !isNaN(totalVAT)
      ? parseFloat(String(order.total_amount)) - totalVAT
      : 0;
  ...
  if (receiptType === 'summary') {
    ...
    return (
      <Paper
        ...
        elevation={3}
        sx={{ p: 3, maxWidth: 400, mx: 'auto', fontFamily: 'monospace', fontSize: '0.875rem' }}
      >
        ...
        <Box sx={{ mb: 2 }}>
          ...
          <Grid container spacing={1}>
            ...
            <Grid item xs={6}>
              <Typography variant="body2">Sous-total HT: {formatCurrency(sousTotalHT)}</Typography>
            </Grid>
          </Grid>
        </Box>
