...
- **Archive route** (`routes/legal/archive.ts`): Uses **establishment context** for all operations (see A3 implementation `docs/patch-notes/69-LEGAL-PER-ESTABLISHMENT-A3-IMPLEMENTATION.md`): `getEstablishmentId`, `requirePermission(access_closure)`, and `ArchiveService` list/get require a **mandatory** `establishment_id` — no global listing of every tenant’s exports. `POST /create` passes `establishment_id` into `ArchiveService.exportData()` so exports stay tenant-scoped.
