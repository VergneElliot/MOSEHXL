...
| **P3-S14** | Extend log redaction to `refresh_token`, `invitation_token` field names | **Fixed (2026-05-27):** request and client-error logging now explicitly redact `refresh_token`/`invitation_token` fields (including nested payload keys) plus `x-refresh-token` header. | **S** |
...
| **P3-Q12** | Expand `@mosehxl/types` and ban `any` in new frontend code | **In progress (2026-05-27):** expanded shared DTOs for catalog API records; removed `any` usage from catalog API adapters, auth invitation flow components, establishment account-creation setup flow types/components, POS payment callback/API hooks, closure API hook casts, establishment service adapters, shared UI table/form generic types, ErrorBoundary reporting/severity UI typing, settings tabs hook props, and product dialog form callbacks; broader frontend `any` cleanup remains. | **L** |
| **P3-Q13** | Toggle `noUncheckedIndexedAccess: true` in both tsconfigs | **In progress (2026-05-27):** enabled in frontend `tsconfig.json` and fixed all resulting type regressions in UI/payment/setup/closure/service code paths; backend toggle attempted and mapped (large error surface), scheduled as next tranche. | **M** |
