...
| **P3-Q12** | Expand `@mosehxl/types` and ban `any` in new frontend code | **In progress (2026-05-27):** expanded shared DTOs for catalog API records; removed `any` usage from catalog API adapters, auth invitation flow components, establishment account-creation setup flow types/components, POS payment callback/API hooks, closure API hook casts, and establishment service adapters; broader frontend `any` cleanup remains. | **L** |
