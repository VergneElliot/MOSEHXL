...
| **P3-Q12** | Expand `@mosehxl/types` and ban `any` in new frontend code | **In progress (2026-05-27):** expanded shared DTOs for catalog API records and removed `any` usage from catalog API adapters, auth invitation flow components, and establishment account-creation setup flow types/components; broader frontend `any` cleanup remains. | **L** |
